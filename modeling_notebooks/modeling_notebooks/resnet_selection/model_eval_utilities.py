...
def generate_metric_curves(
    ...
    """
    Generate a DataFrame with the ROC and PR curves for the given data.
    ...
    for i in range(breaks + 1):
        ...
        df["prediction"] = df[score_colname] > threshold
        confusion_matrix = pd.crosstab(df[true_category_colname], df["prediction"])
